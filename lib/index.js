...
/* NOTE:
 *  A new option for cluster definition named dbClusterConnection.
 *  It defined array of nodes in the cluster in the following structure:
 *     dbClusterConnection =[{
 *        host: <host>,
 *        username: <username>,
 *        password: <password>,
...
var SessionStore = module.exports = function (options, connection, cb) {
    ...
    if (this.options.dbClusterConnection && !connection) {
        var clusterConfig = {
            removeNodeErrorCount: 1, // Remove the node immediately when connection fails.
            defaultSelector: 'ORDER'
        };
        var pool = mysql.createPoolCluster(clusterConfig);
        for (var i = 0; i < this.options.dbClusterConnection.length; i++) {
            var nodeConfig = {
                host: this.options.dbClusterConnection[i].host,
                user: this.options.dbClusterConnection[i].username,
                password: this.options.dbClusterConnection[i].password,
                database: this.options.dbClusterConnection[i].dbName
            };
        }
        ...
        getConnection(pool, function (connection) {
            that.manager = new MySQLConnectionManager(options, connection || null);
            that.connection = that.manager.connection;
            if (!that.options.createDatabaseTable)
                return cb && cb();

            that.createDatabaseTable(cb);
        });
...
function getConnection(pool, cb) {
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            try {
                ...
                console.log('error during getConnection to db . ended with error ' + exception);
                return;
            }
            console.log('error during getConnection to db . ended with error ' + err);
        }
        cb(connection);
    });
}
...
SessionStore.prototype.createDatabaseTable = function (cb) {
    ...
    var fileName = __dirname + '/../schema.sql'
    if(self.options.dbClusterConnection){
        fileName = __dirname + '/../schema_cluster.sql'
    }

    fs.readFile(fileName, 'utf-8', function(error, sql) {
